...
export abstract class QueryBuilder<Entity> {
  ...
  relation(
    ...
    const entityTarget = arguments.length === 2
  ...
  protected replacePropertyNames(statement: string) {
    ...
    for (const alias of this.expressionMap.aliases) {
      ...
      const replacementAliasNamePrefix =
      ...
      if (replacementKeys.length) {
        statement = statement.replace(
          new RegExp(
            ...
            `([ =\(]|^.{0})` +
              `${escapeRegExp(replaceAliasNamePrefix)}(${
                replacementKeys.map(escapeRegExp).join("|")
              })` +
              `(?=[ =\)\,]|.{0}$)`,
            "gm",
          ),
          (_, pre, p) =>
            `${pre}${replacementAliasNamePrefix}${
              this.escape(replacements[p])
            }`,
        );
      }
    }
  ...
  protected createWhereExpression() {
    ...
    if (this.expressionMap.mainAlias!.hasMetadata) {
      ...
      if (
        this.expressionMap.queryType === "select" &&
        !this.expressionMap.withDeleted && metadata.deleteDateColumn
      ) {
  ...
  protected createWhereIdsExpression(ids: any | any[]): string {
    ...
    const whereStrings = normalized.map((id, index) => {
      ...
      metadata.primaryColumns.forEach((primaryColumn, secondIndex) => {
        ...
        this.expressionMap.nativeParameters[parameterName] = primaryColumn
          .getEntityValue(id, true);
        parameterIndex++;
      });
  ...
  /**
     ...
     */
  protected computeWhereParameterColumnRelation(
    column: ColumnMetadata | RelationMetadata,
    where: ObjectLiteral,
    alias: string,
  ): any {
    let propertyPaths = EntityMetadata.createPropertyPath(
      this.expressionMap.mainAlias!.metadata,
      where,
    );

    const columnRelation = column instanceof RelationMetadata
      ? column
    ...
    return propertyPaths.map((propertyPath, propertyIndex) => {
      const columns = columnRelation.inverseEntityMetadata
      ...
      return columns.map((columnInverse, columnIndex) => {
        ...
        if (
          ...
          const relationEagerAlias = this.connection.namingStrategy
            .eagerJoinRelationAlias(alias, columnRelation.propertyPath);
          const relationAlias = this.connection.namingStrategy
            .joinRelationAlias(alias, columnRelation.propertyPath);

          const aliasPath = columnRelation?.isEager
            ? relationEagerAlias
            : relationAlias;

          return this.computeWhereParameterColumnRelation(
            columnInverse,
            whereRelation,
            aliasPath,
          );
        ...
        const relationEagerAlias = this.connection.namingStrategy
          .eagerJoinRelationAlias(
            alias,
            columnRelation?.propertyPath || columnInverse.propertyPath,
          );
        const relationAlias = this.connection.namingStrategy.joinRelationAlias(
          alias,
          column.propertyName,
        );
        const aliasPath = columnRelation?.isEager
          ? `"${relationEagerAlias}"."${propertyPath}"`
          : `"${relationAlias}"."${propertyPath}"`;

        let parameterValue = columnInverse.getEntityValue(where, true) ||
          column.getEntityValue(where, true) || where[propertyPath];
        let parameterIndex =
          Object.keys(this.expressionMap.nativeParameters).length;

        const parameterName =
          `where_${aliasPath}_${propertyIndex}_${columnIndex}`;
        const parameterBaseCount =
          Object.keys(this.expressionMap.nativeParameters).filter((x) =>
          ...
          if (parameterValue.useParameter) {
            if (parameterValue.objectLiteralParameters) {
              ...
              realParameterValues.forEach(
                (realParameterValue, realParameterValueIndex) => {
                  this.expressionMap
                    .nativeParameters[
                      parameterName +
                      (parameterBaseCount + realParameterValueIndex)
                    ] = realParameterValue;
                  parameterIndex++;
                  parameters.push(
                    this.connection.driver.createParameter(
    ...
    if (where instanceof Brackets) {
      ...
      if (this.expressionMap.mainAlias!.hasMetadata) {
        andConditions = wheres.map((where, whereIndex) => {
          ...
          return propertyPaths.map((propertyPath, propertyIndex) => {
            let columns: ColumnMetadata[] | RelationMetadata[] = this
              .expressionMap.mainAlias!.metadata.findColumnsWithPropertyPath(
                propertyPath,
              );

            if (!columns.length) {
              const relation = this.expressionMap.mainAlias!.metadata
            ...
            return columns.map(
              (column: ColumnMetadata | RelationMetadata, columnIndex: any) => {
                const whereRelation = where[propertyPath];
                ...
                if (
                  ...
                  ((column instanceof RelationMetadata && !column.isLazy) ||
                    (column instanceof ColumnMetadata &&
                      column.relationMetadata &&
                ...
                const parameterName = "where_" + whereIndex + "_" +
                  propertyIndex + "_" + columnIndex;
                const parameterBaseCount =
                  Object.keys(this.expressionMap.nativeParameters).filter((x) =>
                    x.startsWith(parameterName)
                  ).length;
  ...
  protected computeFindOperatorExpression(
    ...
    switch (operator.type) {
      case "not":
        if (operator.child) {
          return `NOT(${
            this.computeFindOperatorExpression(
              operator.child,
              aliasPath,
              parameters,
            )
      ...
      case "ilike":
        if (
          driver instanceof PostgresDriver || driver instanceof CockroachDriver
        ) {
